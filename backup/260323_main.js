...
//Code retriever to backend
async function parseIFC(file) {
    // 1. Create a form to send the file
    const formData = new FormData();
    formData.append('ifc', file);
    
    // 2. Send file to Python backend
    const response = await fetch('http://localhost:5000/parse-ifc', {
        method: 'POST',
        body: formData
    });
    
    // 3. Get the result from Python
    const data = await response.json();
    
    // 4. Show it to the user
    //alert(`Doors: ${data.doors}`);
}

// Global subsets variable
let subsets = {};
...
document.getElementById("file-input").addEventListener("change", async (event) => {
    ...
    */
    ...
    // Continue with normal IFC loading
...
// Connect checkboxes to visibility (automatically uses all categories from config)
